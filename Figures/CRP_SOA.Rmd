...
```{r echo=FALSE}
RecPFR.pwc<-PFR %>%
  pairwise_t_test(pfr~recall, paired=TRUE, p.adjust.method='fdr') #%>%
  #select(-df,-statistic) #remove details 
RecPFR.pwc  

PFR %>%
  cohens_d(pfr~recall, paired=TRUE)
...
sp.pwc<-PFR %>%
  pairwise_t_test(pfr~sp, paired=TRUE, p.adjust.method='fdr') #%>%
  #select(-df,-statistic) #remove details 
sp.pwc
PFR %>%
  cohens_d(pfr~sp, paired=TRUE)
...
RR %>%
  cohens_d(relativerecall~oddballtype, paired=FALSE)
...
recall.fig+
  stat_pvalue_manual(WordPositionxOdd.pwc, label='p={p.adj}', inherit.aes=FALSE, hide.ns=TRUE, tip.length=0, bracket.nudge.y=-0.7)+
  stat_pvalue_manual(WordPosition.pwc, label='p={p.adj}', inherit.aes=FALSE, hide.ns=TRUE, tip.length =0, bracket.nudge.y=-0.42)
```
...
### Total recalled items per list per SOA
The goal of this is to check whether the total recalled items per list per SOA increases as SOA increases. 
```{r echo=FALSE}
setwd("/Users/albaperis/Desktop/Alba/PhD UPM /Von Restroff WP3/Paper_github/Odd_SOA_CRP/Raw_Results")
...
list.recall.SOA.fig<-ggplot(data = listrecallSOAplot,mapping = aes(x = as.factor(SOA), y = mean))+
     geom_bar(position='dodge',stat = "identity") +
  geom_bar(position='dodge', stat='identity', fill='mistyrose2')+
  geom_errorbar(aes(x=as.factor(SOA), ymin=mean-ci, ymax=mean+ci), width=0.4, color='coral1',
                size=0.8, position=position_dodge(0.8), stat='identity')+
    ggtitle('Number of items recalled by SOA')+
...
TFCRP %>%
  group_by(oddballtype) %>%
  cohens_d(CRP~transition, paired=TRUE)
```


## Transitions ***to*** and ***from*** the oddballs by SOA
We looked at CRP curves in transitions to and from oddballs by SOA. 
...
TFSOAplot$transition<-factor(TFSOAplot$transition, levels=c('to','from'))
...
```{r echo=FALSE, warning=FALSE}
TFSOA.QQ<-ggqqplot(TFCRPSOA,'CRP',ggtheme=theme_bw())+
  facet_grid(oddballtype+direction+transition+SOA~wordposition, labeller='label_both') # looks fine 
TFSOA.QQ
```
...
plot(effect('oddballtype*wordposition*direction*SOA',test))

#Run paired t tests
# Main effect of oddball type
TFSOA_oddball.pwc<-TFCRPSOA %>%
  pairwise_t_test(CRP~oddballtype, paired=TRUE, p.adjust.method='fdr') #%>%
  #select(-df,-statistic) #remove details 
TFSOA_oddball.pwc
plot(effect('oddballtype',TFSOAplot_glmer))

TFCRPSOA %>%
  cohens_d(CRP~oddballtype, paired=TRUE)

#Main effect of wordposition
TFSOA_wordposition.pwc<-TFCRPSOA %>%
  pairwise_t_test(CRP~wordposition, paired=TRUE, p.adjust.method='fdr') #%>%
  #select(-df,-statistic) #remove details 
TFSOA_wordposition.pwc
plot(effect('wordposition',TFSOAplot_glmer))

TFCRPSOA %>%
  cohens_d(CRP~wordposition, paired=TRUE)


#Interaction oddballtype x transition (important effect)

OddTypexTransition.TF.SOA.pwc<-TFCRPSOA %>%
  group_by(transition)%>%
  pairwise_t_test(CRP~oddballtype, paired=TRUE, p.adjust.method='fdr') #%>%
  #select(-df,-statistic) #remove details 
OddTypexTransition.TF.SOA.pwc
plot(effect('oddballtype*transition',TFSOAplot_glmer))
TFCRPSOA %>%
  group_by(oddballtype) %>%
  cohens_d(CRP~transition, paired=TRUE)

#Oddballtype x direction 
OddballTypexDirection.TF.SOA.pwc<-TFCRPSOA %>%
  group_by(direction)%>%
  pairwise_t_test(CRP~oddballtype, paired=TRUE, p.adjust.method='fdr') #%>%
  #select(-df,-statistic) #remove details 
OddballTypexDirection.TF.SOA.pwc
plot(effect('oddballtype*direction',TFSOAplot_glmer))
TFCRPSOA %>%
  group_by(direction) %>%
  cohens_d(CRP~oddballtype, paired=TRUE)

#Oddball type x wordposition x direction 
#OddxWpxDir.glmer<-glmer(CRP ~ oddballtype*wordposition*direction + (1|subject), data = TFCRPSOA)
#Anova(OddxWpxDir.glmer,3)

plot(effect('oddballtype*wordposition*direction',TFSOAplot_glmer))
#TFCRPSOA %>%
  cohens_d(CRP~SOA, paired=TRUE)

#Oddball type x direction x SOA
#OddxDirxSOA.glmer<-glmer(CRP ~ oddballtype*direction*SOA + (1|subject), data = TFCRPSOA)
#Anova(OddxDirxSOA.glmer,3)
...
#OddxWpxDirxSOA.glmer<-glmer(CRP ~ oddballtype*wordposition*direction*SOA + (1|subject), data = TFCRPSOA)
#Anova(OddxWpxDirxSOA.glmer,3)

plot(effect('oddballtype*wordposition*direction*SOA',TFSOAplot_glmer))
#TFCRPSOA %>%
  #cohens_d(CRP~SOA, paired=TRUE)
...
#### Valence and Arousal on memory (Spanish)
Look at whether there is an effect of valence and arousal on memory for emotional items in the present sample. 
```{r}
setwd("/Users/albaperis/Desktop/Alba/PhD UPM /Von Restroff WP3/Paper_github/Odd_SOA_CRP/Raw_Results")
arval<-read.csv('arvalrec_R.csv')
arval<-as.data.frame(arval)
arval$recall <- as.factor(arval$recall)

#remove E-1 
arval2<-as.data.frame(subset(arval, wordposition=='E'))

arval.glm<-glm(recall~valencemean+arousalmean, family=binomial, data=arval2) #binomial fits a log reg
Anova(arval.glm)
plot(allEffects(arval.glm)) #plot all the effects

#try with summary
#summary(arval.glm)
#plot(effect('valencemean',arval.glm)) #plot 1 effect


#arval.glmer<-glmer(recall~valencemean+arousalmean +(1|subject)+(1|word), family=binomial, data=arval2) #binomial fits a log reg
#Anova(arval.glmer)
#plot(effect('valencemean',arval.glmer))


#arval.glm.stats<-summary(arval.glm)$coef
#arval.glm.stats

#ggplot(arval2, aes(x=valencemean, y=arousalmean, color=recall))+
#  geom_line(lwd=2)

```

#### Predict E-1 recall as a function of valence and arousal on E recall

```{r}
setwd("/Users/albaperis/Desktop/Alba/PhD UPM /Von Restroff WP3/Paper_github/Odd_SOA_CRP/Raw_Results")
Em1arval<-read.csv('Em1arvalrec_R.csv')
Em1arval<-as.data.frame(Em1arval)

Em1arval$E <- as.factor(Em1arval$E)
Em1arval$Em1 <- as.factor(Em1arval$Em1)
e_noe <- glm(Em1 ~ valencemean+arousalmean, data = Em1arval, family= binomial)
e_0 <- glm(Em1 ~ valencemean + E + arousalmean, data = Em1arval, family = binomial) #only me
e_1 <- glm(Em1 ~ valencemean*E + arousalmean*E, data= Em1arval, family=binomial) #interactions
anova(e_noe,e_0,e_1) #is the model with the me different from the one with interactions? No. (Deviance>0.05) and the AIC is lower for e_0 (see if summary())
...
#test<-glm(Em1~E+arousalmean+valencemean, data=Em1arval, family=binomial)
#Anova(test)
#summary(test)

#e_recalled <- subset(Em1arval, E==1)
#e_1_erecalled <- glm(Em1 ~ valencemean + arousalmean,data= e_recalled, family=binomial)
#summary(e_1_erecalled)

#Darya's code:
#e_1_erecalled <- glm(recall_e_1 <- valencemean + arousal,data= e_recalled, family=binomia)

#setwd("/Users/albaperis/Desktop/Alba/PhD UPM /Von Restroff WP3/Paper_github/Odd_SOA_CRP/Raw_Results")
#darya <- read.csv('darya_R.csv')
#darya$Erecall <- factor(darya$Erecall)
#gmixed0<- glmer(Em1recall ~ valencemean+Erecall + (1|subject), data = darya, family=binomial)
#gmixed1<- glmer(Em1recall ~ valencemean*Erecall + (1|subject), data = darya, family=binomial)
#anova(gmixed0,gmixed1)

```

#### Valence and Arousal English v. Spanish
Compare whether there are differences in valence and arousal between the spanish and english version of the items (note that values were obtained from different papers with a different amount of raters each)
```{r echo=FALSE}
#setwd("/Users/albaperis/Desktop/Alba/PhD UPM /Von Restroff WP3/Paper_github/Odd_SOA_CRP/Valence")
#arvalEvS<-as.data.frame(arvalEvS)
#arvalEvS<-read.csv('valence_EvS.csv')

#Get normality values
#arvalnormality<-arvalEvS %>%
 # group_by(language) %>%
  #shapiro_test(valencemean)
  #View()

#Group the variables 
#valence.grouped<-group_by(arvalEvS, language) %>%
 # summarise(
  #  count=n(),
   # mean=mean(valencemean),
    #sd=mean(valencesd)
  #)
#valence.plt<-ggboxplot(arvalEvS, x='language', y='valencemean',
 #                      color='language')
#valence.plt

#arousal.grouped<-group_by(arvalEvS, language) %>%
 # summarise(
  #  count=n(),
   # mean=mean(arousalmean),
    #sd=mean(arousalsd)
  #)
#arousal.plt<-ggboxplot(arvalEvS, x='language', y='arousalmean',
 #                      color='language')
#arousal.plt

#Run t tests
#arousal.ttest<-t.test(arousalmean~language, data=arvalEvS, paired=FALSE)
#valence.ttest<-t.test(valencemean~language, data=arvalEvS, paired=FALSE)
```