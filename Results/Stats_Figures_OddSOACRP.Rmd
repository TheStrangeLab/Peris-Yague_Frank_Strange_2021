...
```{r echo=FALSE}
#factor the columns
PFR$subject<-factor(PFR$subject)
PFR$oddballtype<-factor(PFR$oddballtype)
PFR$recall<-factor(PFR$recall)
PFR$sp<-factor(PFR$sp)
...
# Is there a difference in relative recall position between SOAs?
# Plot 
RR.plot.SOA.data<- RR %>%
...
RR.SOA.outliers<-RR %>%
  group_by(oddballtype,SOA) %>%
  identify_outliers(relativerecall) #test for outliers
...
### Fig. 6 Total recalled items per list per SOA
...
list.recall.SOA.fig<-ggplot(data = listrecallSOAplot,mapping = aes(x = as.factor(SOA), y = mean, fill=SOA))+
  ...
  #geom_jitter(data=listrecallSOA,mapping =aes(x=SOA, y=total_recall_list))+
...
list.recall.SOA.fig + geom_jitter(data=listrecallSOA,mapping =aes(x=SOA, y=total_recall_list), alpha=0.1)#+scale_color_brewer(palette="black")
...
## Fig 3C. Transitions ***to*** and ***from*** the oddballs
...
```{r echo=FALSE, warning=FALSE, message=FALSE}
#E_subset<-TFCRP[which(TFCRP$oddballtype=='E'),]


#E_subset.aov<-anova_test(
#  data=E_subset, dv=CRP, wid=subject,
#  within=c(wordposition,direction,transition)
#)
#get_anova_table(E_subset.aov)
...
test <- lmer(CRP ~ oddballtype*wordposition*direction*transition + (1|subject), data = TFCRP, na.action='na.omit')
Anova(test,3)
test.pwc<-emmeans(test, ~ direction*transition)
pairs(test.pwc)
...
#plotTF.new <- as.data.frame(effect('direction*transition*SOA',TFCRP_collapsed))
