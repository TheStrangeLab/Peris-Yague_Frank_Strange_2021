...
#test<-ggplot(recall,aes(x=wordposition, y=normrec, color=oddballtype))+
 #geom_boxplot(mapping=aes(x=wordposition,y=normrec),stat='summary',size=1)+geom_point(aes(group=interaction(subject,oddballtype)),position = position_dodge(width = .35),alpha=0.1)+
  #scale_fill_manual(values=c('thistle1','lightblue1'), labels=c('Emotional','Perceptual'), name='Oddball Type')+
#  scale_x_discrete(
 #   labels = c("Odd-2", "Odd-1", "Odd","Odd+1")
  #)+
...
ggplot(recall, aes(x=factor(wordposition, labels=c('Odd-2', 'Odd-1', 'Odd', 'Odd+1')), y=normrec, fill=oddballtype))+
...
listrecallSOAplot<-listrecallSOA %>%
  group_by(SOA) %>%
  get_summary_stats(total_recall_list,type='mean_se')
...
list.recall.SOA.fig<-ggplot(data = listrecallSOAplot,mapping = aes(x = as.factor(SOA), y = mean))+
  ...
  geom_errorbar(aes(x=as.factor(SOA), ymin=mean-se, ymax=mean+se), width=0.4, color='coral1',
...
Anova(arval.glm)
plot(allEffects(arval.glm)) #plot all the effecta
...
e_1 <- glm(Em1 ~ valencemean*E + arousalmean*E, data= Em1arval, family=binomial)
summary(e_1)
plot(allEffects(e_1))
